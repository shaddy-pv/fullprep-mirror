  ...
  return (
    <div className="flex items-center justify-between">
      <div className="flex flex-col text-left">
        <span className="text-[13px] font-bold text-white tracking-[-0.01em]">{label}</span>
        <span className="text-[11px] text-[#9ca3af]/90 font-medium">{description}</span>
      </div>
      {children}
    </div>
  );
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title="Editor Settings" className="bg-[#111217] dark:bg-[#06090f] text-white border-white/[0.08]">
      <div className="py-2 space-y-4">
        {/* Theme Selector */}
        <SettingRow label="Editor Theme" description="Configure IDE appearance theme">
          <select
            value={editorTheme}
        ...
        {/* Toggle Settings */}
        <SettingRow label="Word Wrap" description="Wrap long code lines to prevent horizontal scrolling">
          <ToggleSwitch
            enabled={wordWrap === "on"}
            onToggle={() => updateSetting("wordWrap", wordWrap === "on" ? "off" : "on")}
          />
        </SettingRow>

        <SettingRow label="Code Minimap" description="Display code bird&apos;s-eye navigator outline">
          <ToggleSwitch
            enabled={minimapEnabled}
            onToggle={() => updateSetting("minimap", !minimapEnabled)}
          />
        </SettingRow>

        <SettingRow label="Line Numbers" description="Show line counter indexes in left margin">
          <ToggleSwitch
            enabled={lineNumbers === "on"}
            onToggle={() => updateSetting("lineNumbers", lineNumbers === "on" ? "off" : "on")}
          />
        </SettingRow>

        {/* Tab Size Selector */}
        <SettingRow label="Tab Size" description="Tab key indent spacer width size">
          <select
            value={tabSize}
