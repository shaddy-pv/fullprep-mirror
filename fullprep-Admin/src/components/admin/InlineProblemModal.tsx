    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
      <div className="w-full max-w-5xl bg-surface rounded-2xl shadow-xl flex flex-col max-h-[95vh]">
        <div className="flex items-center justify-between p-5 border-b border-border-card">
          ...
          <button onClick={onClose} className="p-1 text-text-muted hover:text-text-primary"><X className="h-6 w-6" /></button>
        </div>
          ...
          <form id="inline-problem-form" onSubmit={submitProblem} className="space-y-8">
            ...
            <Section title="Problem Content">
              ...
              <Field label="Constraints">
                <DynamicArray items={form.constraints as string[]} onAdd={() => addArrayItem("constraints", "")} onRemove={(i) => removeArrayItem("constraints", i)}>
                  {(item, i) => <input value={item} onChange={(e) => updateArrayItem("constraints", i, e.target.value)} className={inputCls} placeholder={`Constraint ${i + 1}`} />}
                </DynamicArray>
              </Field>
            ...
            <Section title="Examples Section">
              <DynamicArray items={form.examples as any[]} onAdd={() => addArrayItem("examples", { input: "", output: "", explanation: "" })} onRemove={(i) => removeArrayItem("examples", i)}>
                {(item, i) => (
                  <div className="grid grid-cols-1 gap-3 md:grid-cols-2 bg-background/50 p-4 rounded-xl border border-border-card">
                    <Field label="Input"><textarea value={item.input} onChange={(e) => updateArrayItem("examples", i, { ...item, input: e.target.value })} rows={3} className={textareaCls} /></Field>
                    <Field label="Output"><textarea value={item.output} onChange={(e) => updateArrayItem("examples", i, { ...item, output: e.target.value })} rows={3} className={textareaCls} /></Field>
            ...
            <div className="grid grid-cols-1 gap-8 md:grid-cols-2">
              <Section title="Public Test Cases (Visible)">
                <DynamicArray items={form.publicTests as any[]} onAdd={() => addArrayItem("publicTests", { input: "", output: "", explanation: "" })} onRemove={(i) => removeArrayItem("publicTests", i)}>
                  {(item, i) => (
              ...
              <Section title="Hidden Test Cases (Secret)">
                <DynamicArray items={form.privateTests as any[]} onAdd={() => addArrayItem("privateTests", { input: "", output: "", explanation: "" })} onRemove={(i) => removeArrayItem("privateTests", i)}>
                  {(item, i) => (
                    <div className="flex gap-2">
            ...
            <Section title="Starter Code Templates">
              <DynamicArray items={form.starterCodeTemplates as any[]} onAdd={() => addArrayItem("starterCodeTemplates", { language: "JAVASCRIPT", code: "" })} onRemove={(i) => removeArrayItem("starterCodeTemplates", i)}>
                {(item, i) => (
        ...
        <div className="p-6 border-t border-border-card flex justify-end gap-3 bg-background/50 rounded-b-2xl">
          <button type="button" onClick={onClose} className="px-5 py-2.5 text-sm font-medium hover:bg-surface-hover rounded-lg border border-border-card">Cancel</button>
          <button form="inline-problem-form" type="submit" disabled={isSubmitting} className="px-5 py-2.5 text-sm font-semibold bg-brand-primary text-primary-foreground rounded-lg disabled:opacity-50 hover:brightness-110">
