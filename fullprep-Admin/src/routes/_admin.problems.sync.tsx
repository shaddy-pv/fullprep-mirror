...
function SyncPage() {
  ...
  const staleCount = problems.filter(
    (p) => !p.lastSyncedAt || Date.now() - new Date(p.lastSyncedAt).getTime() > 24 * 3600_000,
  ).length;
  const missingCount = 282 - problems.length; // Max known from Codnite
  const lastSynced = problems
    .filter((p) => p.lastSyncedAt)
    .sort((a, b) => +new Date(b.lastSyncedAt!) - +new Date(a.lastSyncedAt!))[0];

  const isRunning = activeJob?.status === "RUNNING";
  const progress =
    isRunning && activeJob?.totalToSync > 0
      ? Math.round(
          ((activeJob.syncedCount + activeJob.skippedCount + activeJob.failedCount) /
            activeJob.totalToSync) *
            100,
        )
      : 0;
    ...
    <div className="space-y-6 pb-12">
      <div>
        ...
        <p className="text-sm text-text-muted">
          Manage background synchronization with the upstream Codnite problem catalog.
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-4">
        ...
        <StatBox
          icon={AlertCircle}
          label="Missing Problems"
          value={formatNumber(Math.max(0, missingCount))}
          tone={missingCount > 0 ? "rose" : "primary"}
        />
        <StatBox
          icon={Server}
          label="Stale (>24h)"
          value={formatNumber(staleCount)}
          tone={staleCount > 0 ? "amber" : "primary"}
        />
        <StatBox
          icon={Clock}
          label="Last Synced"
          value={lastSynced?.lastSyncedAt ? formatRelativeTime(lastSynced.lastSyncedAt) : "never"}
        />
      </div>
            ...
            <div className="flex-1 flex flex-col min-h-0 bg-background border border-border-card rounded-xl overflow-hidden">
              <div className="p-4 border-b border-border-card bg-surface/50 grid grid-cols-4 gap-4 text-center">
                <div>
                  ...
                  <div
                    className={`text-sm font-semibold ${activeJob.status === "RUNNING" ? "text-brand-primary" : activeJob.status === "FAILED" ? "text-brand-rose" : "text-brand-emerald"}`}
                  >
                    {activeJob.status}
                  </div>
                </div>
              ...
              <div className="flex-1 p-4 overflow-y-auto font-mono text-[11px] text-text-secondary space-y-1.5">
                {activeJob.logs?.map((l: string, i: number) => {
                  ...
                  return (
                    <div
                      key={i}
                      className={isErr ? "text-brand-rose" : isSys ? "text-brand-primary" : ""}
                    >
            ...
            {history.length > 0 ? (
              <div className="space-y-3">
                {history.map((job: any) => (
                  <div
                    key={job._id}
                    className="p-3.5 rounded-xl border border-border-card bg-background flex flex-col gap-2"
                  >
                    <div className="flex justify-between items-center">
                      <div className="flex items-center gap-2">
                        {job.status === "COMPLETED" ? (
                          ...
                          <AlertCircle className="h-4 w-4 text-brand-rose" />
                        ) : (
                          <RefreshCcw className="h-4 w-4 text-brand-primary animate-spin" />
                        )}
                    ...
                    <div className="grid grid-cols-4 text-xs text-center border-t border-border-card pt-2 mt-1">
                      ...
                      <div>
                        <span className="text-text-muted block mb-0.5">Synced</span>
                        <span className="text-brand-emerald">{job.syncedCount}</span>
                      </div>
                      <div>
                        <span className="text-text-muted block mb-0.5">Skipped</span>
                        <span className="text-text-secondary">{job.skippedCount}</span>
                      </div>
                      <div>
                        <span className="text-text-muted block mb-0.5">Failed</span>
                        <span
                          className={
                            job.failedCount > 0 ? "text-brand-rose" : "text-text-secondary"
                          }
                        >
                          {job.failedCount}
                        </span>
