...
function AnalyticsPage() {
  ...
  const dau = new Set(subs.filter((s) => new Date(s.createdAt) >= today).map((s) => s.user._id))
    .size;
  const wau = new Set(subs.filter((s) => new Date(s.createdAt) >= weekStart).map((s) => s.user._id))
    .size;
  ...
  // Top Active Users
  const userActivityCount = filteredSubs.reduce<
    Record<string, { name: string; count: number; id: string }>
  >((acc, s) => {
  ...
  const diffData = Object.entries(diffBuckets)
    .map(([name, value]) => ({ name, value }))
    .sort((a, b) => b.value - a.value);

  const probCounts = filteredSubs.reduce<
    Record<string, { name: string; attempts: number; id: string }>
  >((acc, s) => {
    if (!acc[s.problemExternalId])
      acc[s.problemExternalId] = {
        name: s.problemName || s.problemExternalId,
        attempts: 0,
        id: s.problemExternalId,
      };
    acc[s.problemExternalId].attempts++;
    return acc;
  }, {});
  const topProblems = Object.values(probCounts)
    .sort((a, b) => b.attempts - a.attempts)
    .slice(0, 8)
    .map((p) => ({ ...p, name: p.name.slice(0, 25) }));
  ...
  const acceptanceRate =
    totalFilteredSubs > 0 ? ((acceptedSubs / totalFilteredSubs) * 100).toFixed(1) : "0";
  ...
  const langBuckets = filteredSubs.reduce<
    Record<string, { count: number; totalTime: number; totalMem: number; acc: number }>
  >((acc, s) => {
  ...
  const langData = Object.entries(langBuckets)
    .map(([name, data]) => ({
      name,
      value: data.count,
  ...
  const avgJudgeTime =
    filteredSubs.reduce((acc, s) => acc + (s.executionTimeMs || 0), 0) / (filteredSubs.length || 1);
  ...
  if (loadingUsers || loadingProblems || loadingSubs)
    return (
    ...
    <div className="space-y-6 pb-12">
      ...
      <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-4 bg-surface p-5 rounded-2xl border border-border-card">
        <div>
          ...
          <p className="text-sm text-text-muted mt-1">
            Platform-wide metrics, judge health, and user insights.
          </p>
        </div>
      ...
      <div className="grid grid-cols-2 gap-4 xl:grid-cols-5">
        ...
        <StatCard
          ...
          icon={Target}
          tone={Number(acceptanceRate) > 50 ? "primary" : "amber"}
          index={2}
          hint={`Last ${timeFilter}d`}
        />
        <StatCard
        ...
        <StatCard
          ...
          icon={Server}
          tone={queueLength > 0 ? "rose" : "secondary"}
          index={4}
          hint={queueLength > 0 ? "Processing..." : "Idle"}
        />
      </div>
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
        ...
        <div className="lg:col-span-1">
          <ChartCard
            ...
            subtitle={`Filtered by last ${timeFilter} days`}
          >
              ...
              <PieChart margin={{ top: 0, right: 0, left: 0, bottom: 0 }}>
                ...
                >
                  {verdictData.map((entry, index) => (
                    <Cell
                      key={index}
                      fill={VERDICT_COLORS[entry.name] || "#aaa"}
        ...
        <div className="lg:col-span-2">
          <ChartCard
            ...
            subtitle="Total vs Accepted submissions over time"
          >
            <ResponsiveContainer width="100%" height="100%">
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
          ...
          <ResponsiveContainer width="100%" height="100%">
            <LineChart data={userGrowthData} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
              ...
              <Line
                ...
                strokeWidth={3}
                dot={false}
                activeDot={{ r: 6, strokeWidth: 0, fill: "var(--color-brand-primary)" }}
              />
            </LineChart>
          </ResponsiveContainer>
        </ChartCard>
        ...
        <ChartCard
          ...
          subtitle="Avg attributes across languages (Mocked Scales)"
        >
          <ResponsiveContainer width="100%" height="100%">
            <RadarChart
              cx="50%"
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
        ...
        <div className="lg:col-span-2">
          <ChartCard
            className="h-[340px]"
            title="Top Attempted Problems"
            subtitle={`By submission count (Last ${timeFilter} days)`}
          >
            <ResponsiveContainer width="100%" height="100%">
              <BarChart
                data={topProblems}
        ...
        <div className="lg:col-span-1">
          <ChartCard
            ...
            subtitle="Real-time system diagnostics"
          >
            <div className="flex flex-col h-full justify-between pb-2">
              <div className="space-y-5 pt-2">
                ...
                />
                <HealthMetric
                  label="Judge Avg Time"
                  value={`${Math.round(avgJudgeTime)}ms`}
                  status="good"
                  icon={Cpu}
                />
