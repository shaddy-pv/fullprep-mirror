    ...
    <div className="mx-auto max-w-5xl space-y-6 pb-12">
      ...
      <div>
        ...
        <p className="text-sm text-text-muted">
          {isEditing
            ? `Editing: ${form.name || edit}`
            : "Author a fully complete competitive programming problem."}
        </p>
      </div>
        ...
        <form onSubmit={onSubmit} className="space-y-8">
            ...
            <div className="grid grid-cols-2 gap-4 sm:grid-cols-4">
              ...
              <Field label="Source">
                <select
                  ...
                  className={inputCls}
                >
                  {["CODEFORCES", "CODECHEF", "HACKEREARTH", "LEETCODE", "FULLPREP", "UNKNOWN"].map(
                    (s) => (
              ...
              {(item, i) => (
                <div className="space-y-2">
                  ...
                  <textarea
                    value={item.code}
                    onChange={(e) =>
                      updateArrayItem("starterCodeTemplates", i, { ...item, code: e.target.value })
                    }
                    rows={4}
                    className={textareaCls}
            ...
            <div className="space-y-4">
              <Field label="Title">
                <input
                  ...
                  onChange={(e) =>
                    updateForm("editorial", { ...form.editorial, title: e.target.value })
                  }
                  className={inputCls}
                  placeholder="e.g. Optimal O(N) using HashMap"
                />
              </Field>
              <Field label="Content (Markdown)">
                <textarea
                  value={form.editorial?.content || ""}
                  onChange={(e) =>
                    updateForm("editorial", { ...form.editorial, content: e.target.value })
                  }
                  rows={5}
                  className={textareaCls}
              ...
              <div className="grid grid-cols-2 gap-4">
                <Field label="Time Complexity">
                  <input
                    ...
                    onChange={(e) =>
                      updateForm("editorial", { ...form.editorial, timeComplexity: e.target.value })
                    }
                    className={inputCls}
                    placeholder="e.g. O(N)"
                  />
          ...
          <Section title="Hints Section">
            ...
            >
              {(item, i) => (
                <input
                  value={item}
                  onChange={(e) => updateArrayItem("hints", i, e.target.value)}
                  className={inputCls}
              ...
              <div className="space-y-4">
                <Field label="Time Limit (Seconds)">
                  <input
                    ...
                    onChange={(e) =>
                      updateForm("judgeConfig", { ...form.judgeConfig, timeLimit: +e.target.value })
                    }
                    className={inputCls}
                  />
                </Field>
            ...
            <Section title="Metadata">
              <div className="space-y-4">
                ...
                <Field label="Original Problem Link">
                  <input
                    value={form.originalProblemLink || ""}
                    onChange={(e) => updateForm("originalProblemLink", e.target.value)}
                    className={inputCls}
          ...
          <div className="flex justify-end gap-3 border-t border-border-card pt-6">
            ...
            >
              ...
              {isSubmitting ? "Saving..." : isEditing ? "Save Changes" : "Create Problem"}
            </button>
          </div>
        </form>
      )}
    ...
    <div className="p-8 rounded-[24px] border border-border-card bg-card-bg shadow-sm space-y-8 select-text">
      <div>
        ...
        <div className="flex items-center gap-3 text-xs font-semibold">
          <span className="text-brand-primary px-2 py-1 rounded bg-brand-primary/10">
            {problem.difficulty}
          </span>
          <span className="text-text-secondary border border-border-card px-2 py-1 rounded">
            {problem.source}
          </span>
        </div>
      </div>
      ...
      <div className="space-y-6 text-[14px] text-text-primary leading-relaxed font-medium">
        ...
        {problem.inputFormat && (
          <div>
            <h3 className="font-bold text-[15px] mb-2 uppercase text-text-secondary tracking-wider">
              Input
            </h3>
        ...
        {problem.examples && problem.examples.length > 0 && (
          <div className="space-y-4">
            <h3 className="font-bold text-[15px] uppercase text-text-secondary tracking-wider">
              Examples
            </h3>
          ...
          <div>
            ...
            <ul className="list-disc pl-5 space-y-1 text-[13px]">
              {problem.constraints.map((c, i) =>
                c ? (
                  <li key={i}>
                    <code className="font-mono bg-background/50 px-1.5 py-0.5 rounded border border-border-card">
                      {c}
                    </code>
                  </li>
                ) : null,
