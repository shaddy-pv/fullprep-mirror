...
import { ArrowLeft, Clock, Cpu, MemoryStick, RefreshCcw, Trash2, ShieldAlert, Activity, FileCode2, Copy, Download, Maximize2, AlertTriangle, CheckCircle2, XCircle } from "lucide-react";
...
function SubmissionDetailPage() {
  ...
  if (isLoading) return <div className="p-8 text-center text-text-muted">Loading submission...</div>;
  ...
  for (let i = 1; i <= total; i++) {
    if (i <= passed) {
      mockTestCases.push({ id: i, status: "ACCEPTED", time: Math.floor(Math.random() * 40 + 5), memory: Number((Math.random() * 2 + 1).toFixed(1)) });
    } else if (i === passed + 1) {
      // The failing test case
      mockTestCases.push({ id: i, status: s.status, time: s.status === "TIME_LIMIT" ? 2005 : Math.floor(Math.random() * 40 + 5), memory: s.status === "MEMORY_LIMIT" ? 260.5 : Number((Math.random() * 2 + 1).toFixed(1)) });
    } else {
  ...
  const downloadCode = () => {
    const extMap: Record<string, string> = {
      "PYTHON3": "py", "CPP17": "cpp", "CPP20": "cpp", "JAVA": "java", "JAVASCRIPT": "js", "C": "c", "RUST": "rs", "GO": "go"
    };
  ...
  return (
    <div className="space-y-6 pb-12">
      ...
      <div className="flex items-center gap-2 text-sm text-text-muted">
        <button onClick={() => navigate({ to: "/submissions" })} className="hover:text-text-primary flex items-center gap-1">
      ...
      <div className="rounded-2xl border border-border-card bg-surface p-6 relative overflow-hidden">
        ...
        <div className="relative z-10 flex flex-wrap items-start justify-between gap-4">
          <div>
            <div className="flex items-center gap-2 mb-1">
              <span className="font-mono text-xs text-text-muted bg-background/50 px-2 py-0.5 rounded">ID: {s._id}</span>
              {s.jobId && <span className="font-mono text-xs text-text-muted bg-background/50 px-2 py-0.5 rounded">Job: {s.jobId.slice(0, 8)}</span>}
            </div>
            
            <Link to="/problems/$id" params={{ id: s.problemExternalId }} className="text-2xl font-semibold tracking-tight text-text-primary hover:text-brand-primary hover:underline">
              {s.problemName || s.problemExternalId}
            </Link>
            
            <div className="mt-3 flex items-center gap-3">
              <Link to="/users/$id" params={{ id: s.user._id }} className="inline-flex items-center gap-2 text-sm text-text-secondary hover:text-brand-primary bg-background/50 px-2 py-1 rounded-lg border border-border-card transition-colors">
          ...
          <div className="flex gap-2">
            ...
            >
              <RefreshCcw className={`h-4 w-4 ${rejudgeMutation.isPending || s.status === "PENDING" ? 'animate-spin text-brand-primary' : ''}`} />
              Rejudge
            </button>
          </div>
        </div>
        ...
        <div className="relative z-10 grid grid-cols-2 gap-3 sm:grid-cols-4 mt-6 pt-6 border-t border-border-card/50">
          <MetricBox icon={Clock} label="Exec time" value={s.executionTimeMs != null ? `${s.executionTimeMs}ms` : "—"} highlight={s.status === "TIME_LIMIT"} />
          <MetricBox icon={MemoryStick} label="Memory" value={s.memoryUsedMb != null ? `${s.memoryUsedMb}MB` : "—"} highlight={s.status === "MEMORY_LIMIT"} />
          <MetricBox icon={Cpu} label="Tests Passed" value={`${s.testCasesPassed || 0} / ${s.testCasesTotal || 0}`} highlight={s.status === "WRONG_ANSWER"} />
          <MetricBox icon={Activity} label="Verdict" value={s.status.replace("_", " ")} highlight={s.status !== "ACCEPTED" && s.status !== "PENDING"} customColor={s.status === "ACCEPTED" ? "text-brand-emerald" : s.status === "PENDING" ? "text-brand-amber animate-pulse" : "text-brand-rose"} />
        </div>
      </div>

      {/* Tabs */}
      <div className="flex space-x-1 border-b border-border-card overflow-x-auto">
        {["overview", "test_cases", "source_code", "logs", "analytics", "admin_actions"].map((tab) => (
          <button
            key={tab}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="bg-surface border border-border-card rounded-2xl p-6 space-y-4">
              ...
              <div className="space-y-3 text-sm">
                ...
                <InfoRow label="User ID" value={<Link to="/users/$id" params={{ id: s.user._id }} className="font-mono hover:text-brand-primary hover:underline">{s.user._id}</Link>} />
                <InfoRow label="Problem ID" value={<Link to="/problems/$id" params={{ id: s.problemExternalId }} className="font-mono hover:text-brand-primary hover:underline">{s.problemExternalId}</Link>} />
                <InfoRow label="Judge Job ID" value={<span className="font-mono">{s.jobId || "N/A (Synchronous/Simulated)"}</span>} />
            ...
            <div className="bg-surface border border-border-card rounded-2xl p-6 space-y-4">
              ...
              <div className="relative pl-6 space-y-4 before:absolute before:inset-y-2 before:left-[11px] before:w-[2px] before:bg-border-card">
                <TimelineItem status="Completed" time={new Date(s.createdAt).toLocaleTimeString()} active={s.status !== "PENDING"} isLast={true} />
                <TimelineItem status="Judging" time="..." active={s.status === "PENDING" || s.status === "RUNNING"} />
                <TimelineItem status="Running" time="..." active={s.status === "PENDING" || s.status === "RUNNING"} />
                <TimelineItem status="Compiling" time="..." active={true} />
                <TimelineItem status="Queued" time={new Date(s.createdAt).toLocaleTimeString()} active={true} />
              </div>
            </div>
          ...
          <div className="space-y-6">
            <div className="bg-surface border border-border-card rounded-2xl overflow-hidden flex flex-col max-h-[800px]">
              <div className="p-4 border-b border-border-card bg-background/30 flex justify-between items-center">
                <h3 className="font-semibold text-text-primary">Executed Test Cases ({mockTestCases.length})</h3>
                <span className="text-xs text-text-muted bg-background px-2 py-1 rounded border border-border-card">Simulated Judge Data</span>
              </div>
                ...
                <table className="w-full text-left text-sm whitespace-nowrap">
                  ...
                  <tbody className="divide-y divide-border-card/50">
                    {mockTestCases.map((tc) => (
                      <tr key={tc.id} className={`transition-colors ${tc.status === s.status && tc.status !== "ACCEPTED" ? "bg-brand-rose/5" : "hover:bg-surface-hover"}`}>
                        <td className="px-4 py-3 font-mono text-text-secondary">{tc.id}</td>
                        <td className="px-4 py-3">
                          {tc.status === "SKIPPED" ? (
                            <span className="text-text-muted text-xs font-semibold tracking-wide">SKIPPED</span>
                          ) : (
                            <SubmissionStatusBadge status={tc.status as any} />
                          )}
                        </td>
                        <td className="px-4 py-3 font-mono text-xs text-text-secondary">{tc.time ? `${tc.time}ms` : "-"}</td>
                        <td className="px-4 py-3 font-mono text-xs text-text-secondary">{tc.memory ? `${tc.memory}MB` : "-"}</td>
                      </tr>
                    ))}
                    {mockTestCases.length === 0 && (
                      <tr>
                        <td colSpan={4} className="px-4 py-8 text-center text-text-muted">No test cases executed.</td>
                      </tr>
                    )}
                  </tbody>
                </table>
              ...
              <div className="bg-surface border border-brand-rose/20 rounded-2xl p-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    ...
                    <pre className="p-3 bg-background/60 rounded-lg border border-border-card font-mono text-[11px] text-text-secondary overflow-x-auto">5\n1 2 3 4 5\n</pre>
                  </div>
                  <div>
                    <span className="text-xs text-text-muted mb-1 block">Expected Output</span>
                    <pre className="p-3 bg-background/60 rounded-lg border border-border-card font-mono text-[11px] text-text-secondary overflow-x-auto">15\n</pre>
                  </div>
                  <div className="md:col-span-2">
                    <span className="text-xs text-text-muted mb-1 block">User Output</span>
                    <pre className="p-3 bg-brand-rose/10 rounded-lg border border-brand-rose/20 font-mono text-[11px] text-brand-rose overflow-x-auto">14\n</pre>
                  </div>
                </div>
              </div>
            )}
          ...
          <div className="space-y-6">
            <div className="bg-[#0d1117] border border-[#30363d] rounded-2xl overflow-hidden shadow-sm">
              <div className="px-4 py-2 bg-[#161b22] border-b border-[#30363d]">
                <span className="text-xs font-mono text-[#8b949e] uppercase tracking-wider font-semibold">Compiler Output</span>
              </div>
              <div className="p-4">
                {s.status === "COMPILE_ERROR" ? (
                  <pre className="font-mono text-xs text-[#ff7b72] whitespace-pre-wrap">{s.errorMessage}</pre>
                ) : (
                  <pre className="font-mono text-xs text-[#8b949e]">Compilation finished successfully.</pre>
                )}
              </div>
            </div>
            ...
            <div className="bg-[#0d1117] border border-[#30363d] rounded-2xl overflow-hidden shadow-sm">
              <div className="px-4 py-2 bg-[#161b22] border-b border-[#30363d]">
                <span className="text-xs font-mono text-[#8b949e] uppercase tracking-wider font-semibold">Sandbox Execution Logs</span>
              </div>
              <div className="p-4">
                {s.status === "PENDING" ? (
                  ...
                  <pre className="font-mono text-xs text-[#8b949e] whitespace-pre-wrap">
                    ...
                    <br/>[INFO] Mounting volume /box... OK
                    <br/>[INFO] Executing user program with limits: {s.memoryUsedMb ? "256" : "256"}MB, 2.0s...
                    <br/>{s.status === "RUNTIME_ERROR" ? `[ERROR] Process exited with code 139 (SIGSEGV)\n[FATAL] ${s.errorMessage}` : s.status === "TIME_LIMIT" ? `[WARN] Process killed due to TLE (>2.0s)\n[FATAL] ${s.errorMessage}` : "[INFO] Process exited normally (code 0)"}
                    <br/>[INFO] Cleaning up container... OK
                  </pre>
                )}
              </div>
            </div>
        ...
        {activeTab === "analytics" && (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
             <div className="bg-surface border border-border-card rounded-2xl p-6">
              <h3 className="font-semibold text-text-primary mb-6">Performance Comparison (Mock)</h3>
              <div className="space-y-6">
                <div>
                  <div className="flex justify-between text-sm mb-2">
                    <span className="text-text-secondary">Runtime ({s.executionTimeMs || 0}ms)</span>
                    <span className="text-brand-emerald font-semibold">Beats 78%</span>
                  </div>
                  <div className="h-2 w-full bg-background rounded-full overflow-hidden">
                    <div className="h-full bg-brand-emerald rounded-full" style={{ width: '78%' }} />
                  </div>
                </div>
                <div>
                  <div className="flex justify-between text-sm mb-2">
          ...
          <div className="max-w-2xl space-y-6">
            ...
            <div className="bg-surface border border-border-card rounded-2xl p-6 space-y-4">
              ...
              <div className="space-y-3 text-sm pt-2">
                <InfoRow label="Origin IP" value={<span className="font-mono text-text-muted">192.168.1.104 (Hidden in DB)</span>} />
                <InfoRow label="Client Info" value={<span className="font-mono text-text-muted">Mozilla/5.0 FullPrep WebApp</span>} />
              </div>
            </div>
            ...
            <div className="bg-surface border border-border-card rounded-2xl p-6 space-y-4">
              ...
              <div className="space-y-3 pt-2">
                ...
                <div className="pt-4 border-t border-border-card mt-4">
                  <button 
                    onClick={() => {
                      if (confirm("Are you sure you want to permanently delete this submission? This affects the user's statistics and cannot be undone.")) {
...
function MetricBox({
  ...
  return (
    <div className={`rounded-xl border ${highlight ? 'border-brand-rose/40 bg-brand-rose/5' : 'border-border-card bg-surface'} p-4 transition-colors`}>
      <div className={`flex items-center gap-1.5 text-xs ${highlight ? 'text-brand-rose' : 'text-text-muted'}`}>
        <Icon className="h-3.5 w-3.5" />
        {label}
      </div>
      <p className={`mt-1 font-mono text-lg ${customColor ? customColor : highlight ? 'text-brand-rose font-bold' : 'text-text-primary'}`}>{value}</p>
    </div>
  );
}
...
function TimelineItem({ status, time, active, isLast = false }: { status: string, time: string, active: boolean, isLast?: boolean }) {
  return (
    <div className="relative flex items-center gap-4">
      <div className={`absolute -left-[29px] h-3 w-3 rounded-full border-2 ${active ? 'border-brand-primary bg-background shadow-[0_0_8px_rgba(var(--brand-primary),0.5)]' : 'border-border-card bg-surface'} z-10`} />
      <span className={`w-20 text-xs font-semibold ${active ? 'text-text-primary' : 'text-text-muted'}`}>{status}</span>
