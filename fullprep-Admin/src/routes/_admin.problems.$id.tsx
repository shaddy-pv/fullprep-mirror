...
import { ArrowLeft, Save, Trash2, Clock, MemoryStick, Users, Activity, CheckCircle2, XCircle, Code2, AlertTriangle, RefreshCcw } from "lucide-react";
...
function ProblemDetailsPage() {
  ...
  const { data: p, isLoading: isProblemLoading, error } = useQuery({
    queryKey: ["problem", id],
  ...
  if (error || !p) {
    return (
      <div className="p-8 text-center space-y-4">
        ...
        <button onClick={() => navigate({ to: "/problems" })} className="text-brand-primary hover:underline">
          Back to Problems
        </button>
      </div>
    );
  ...
  const languageDistribution = submissions.reduce((acc, s) => {
  ...
  const subColumns: Column<AdminSubmission>[] = [
    ...
    },
    {
      key: "time",
      header: "Time",
      cell: (s) => <span className="text-xs text-text-secondary">{s.executionTimeMs ? `${s.executionTimeMs}ms` : "-"}</span>,
    },
    {
      key: "memory",
      header: "Memory",
      cell: (s) => <span className="text-xs text-text-secondary">{s.memoryUsedMb ? `${s.memoryUsedMb}MB` : "-"}</span>,
    },
    {
      key: "date",
      header: "Date",
  ...
  return (
    <div className="space-y-6 pb-12">
      ...
      <div className="flex items-center gap-2 text-sm text-text-muted">
        <button onClick={() => navigate({ to: "/problems" })} className="hover:text-text-primary flex items-center gap-1">
        ...
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div>
            <div className="flex items-center gap-2 mb-1">
              <span className="font-mono text-xs text-text-muted bg-background/50 px-2 py-0.5 rounded">ID: {p.externalId}</span>
              <span className="font-mono text-xs text-text-muted bg-background/50 px-2 py-0.5 rounded">Serial: #{p.serialNo}</span>
            </div>
          ...
          <div className="flex flex-col items-end gap-2 text-xs text-text-muted">
            ...
            <span>Last Synced: {p.lastSyncedAt ? formatRelativeTime(p.lastSyncedAt) : "Never"}</span>
            <Link to="/problems/create" search={{ clone: p._id }} className="flex items-center gap-1 text-brand-primary hover:underline mt-2">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="bg-surface border border-border-card rounded-2xl p-6 space-y-4">
              ...
              <div className="space-y-3 text-sm">
                ...
                <InfoRow label="Original Link" value={p.originalProblemLink ? <a href={p.originalProblemLink} target="_blank" rel="noreferrer" className="text-brand-primary hover:underline">View Source</a> : "N/A"} />
                <InfoRow label="Author (Created By)" value={p.createdBy || "System Sync"} />
                <div className="pt-3 border-t border-border-card">
                  <span className="block text-xs text-text-muted mb-2 uppercase tracking-wider font-semibold">Tags</span>
                  <div className="flex flex-wrap gap-1">
                    {p.cfTags.length > 0 ? p.cfTags.map(t => <Pill key={t} tone="neutral">{t}</Pill>) : <span className="text-text-muted">No tags</span>}
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="bg-surface border border-border-card rounded-2xl p-6 space-y-4">
              ...
              <div className="space-y-3 text-sm">
                <InfoRow label="Time Limit" value={`${p.judgeConfig?.timeLimit || p.timeLimitSeconds} seconds`} />
                <InfoRow label="Memory Limit" value={`${p.judgeConfig?.memoryLimit || p.memoryLimitMb} MB`} />
                <InfoRow label="Output Strategy" value={p.judgeConfig?.outputMatchingStrategy || "Exact Match"} />
                <div className="pt-3 border-t border-border-card">
                  <span className="block text-xs text-text-muted mb-2 uppercase tracking-wider font-semibold">Constraints</span>
                  {p.constraints && p.constraints.length > 0 ? (
                    <ul className="list-disc list-inside text-text-primary space-y-1">
                      {p.constraints.map((c, i) => <li key={i} dangerouslySetInnerHTML={{__html: c}}></li>)}
                    </ul>
                  ) : (
        ...
        {activeTab === "content" && (
          <div className="space-y-6">
            <div className="bg-surface border border-border-card rounded-2xl p-6">
              ...
              <div className="prose prose-invert max-w-none text-sm text-text-secondary bg-background/50 p-4 rounded-xl border border-border-card overflow-x-auto" dangerouslySetInnerHTML={{ __html: p.description || "No description provided." }} />
            </div>

            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="bg-surface border border-border-card rounded-2xl p-6">
                <h3 className="font-semibold text-text-primary mb-4">Input Format</h3>
                <div className="text-sm text-text-secondary bg-background/50 p-4 rounded-xl border border-border-card" dangerouslySetInnerHTML={{ __html: p.inputFormat || "Not specified." }} />
              </div>
              <div className="bg-surface border border-border-card rounded-2xl p-6">
                <h3 className="font-semibold text-text-primary mb-4">Output Format</h3>
                <div className="text-sm text-text-secondary bg-background/50 p-4 rounded-xl border border-border-card" dangerouslySetInnerHTML={{ __html: p.outputFormat || "Not specified." }} />
              </div>
            </div>
            ...
            {p.notes && (
              <div className="bg-surface border border-border-card rounded-2xl p-6">
                ...
                <div className="text-sm text-text-secondary bg-background/50 p-4 rounded-xl border border-border-card" dangerouslySetInnerHTML={{ __html: p.notes }} />
              </div>
            )}
          </div>
        )}
          ...
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
              <div className="bg-surface border border-border-card rounded-xl p-4 text-center">
                <span className="text-2xl font-bold text-text-primary">{p.publicTests?.length || 0}</span>
                <span className="block text-xs text-text-muted mt-1 uppercase tracking-wider font-semibold">Public Tests</span>
              </div>
              <div className="bg-surface border border-border-card rounded-xl p-4 text-center">
                <span className="text-2xl font-bold text-text-primary">{p.privateTests?.length || 0}</span>
                <span className="block text-xs text-text-muted mt-1 uppercase tracking-wider font-semibold">Hidden Tests</span>
              </div>
              <div className="bg-surface border border-border-card rounded-xl p-4 text-center">
                <span className="text-2xl font-bold text-text-primary">{p.generatedTests?.length || 0}</span>
                <span className="block text-xs text-text-muted mt-1 uppercase tracking-wider font-semibold">Generated Tests</span>
              </div>
            </div>
            ...
            <div className="bg-surface border border-border-card rounded-2xl overflow-hidden">
              ...
              <div className="divide-y divide-border-card/60">
                {p.examples && p.examples.length > 0 ? p.examples.map((ex, i) => (
                  <div key={i} className="p-4">
                    <div className="text-xs font-semibold text-text-muted mb-2 uppercase">Example {i + 1}</div>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <div>
                        <span className="text-xs text-text-muted mb-1 block">Input</span>
                        <pre className="p-3 bg-background/60 rounded-lg border border-border-card font-mono text-[11px] text-text-secondary overflow-x-auto">{ex.input}</pre>
                      </div>
                      <div>
                        <span className="text-xs text-text-muted mb-1 block">Output</span>
                        <pre className="p-3 bg-background/60 rounded-lg border border-border-card font-mono text-[11px] text-text-secondary overflow-x-auto">{ex.output}</pre>
                      </div>
                    </div>
                    {ex.explanation && (
                      <div className="mt-3 text-xs text-text-secondary bg-brand-primary/5 p-3 rounded-lg border border-brand-primary/10">
                        <span className="font-semibold text-brand-primary">Explanation:</span> <span dangerouslySetInnerHTML={{__html: ex.explanation}} />
                      </div>
                    )}
                  </div>
                )) : (
                  <div className="p-6 text-center text-sm text-text-muted">No examples configured.</div>
                )}
              </div>
            </div>
            ...
            <div className="bg-surface border border-border-card rounded-2xl overflow-hidden">
              ...
              <div className="p-4 space-y-4">
                {p.starterCodeTemplates && p.starterCodeTemplates.length > 0 ? p.starterCodeTemplates.map((t, i) => (
                  <div key={i}>
                    <Pill tone="sky" className="mb-2">{t.language}</Pill>
                    <pre className="p-3 bg-background/60 rounded-lg border border-border-card font-mono text-[11px] text-text-secondary overflow-x-auto">{t.code}</pre>
                  </div>
                )) : (
                  <div className="text-sm text-text-muted text-center py-4">No starter code templates configured. Users will start with a blank editor.</div>
                )}
              </div>
            </div>
            ...
            {p.hints && p.hints.length > 0 && (
              <div className="bg-surface border border-border-card rounded-2xl p-6">
                ...
                <ul className="space-y-2 text-sm text-text-secondary list-decimal list-inside">
                  {p.hints.map((hint, i) => <li key={i} dangerouslySetInnerHTML={{__html: hint}}></li>)}
                </ul>
              </div>
            )}
            ...
            {p.editorial ? (
              <div className="bg-surface border border-border-card rounded-2xl p-6">
                <div className="flex justify-between items-center mb-4">
                  <h3 className="font-semibold text-text-primary">{p.editorial.title || "Editorial"}</h3>
                  <div className="flex gap-2">
                    <Pill tone="rose">Time: {p.editorial.timeComplexity || "O(?)"}</Pill>
                    <Pill tone="sky">Space: {p.editorial.spaceComplexity || "O(?)"}</Pill>
                  </div>
                </div>
                <div className="prose prose-invert max-w-none text-sm text-text-secondary bg-background/50 p-4 rounded-xl border border-border-card" dangerouslySetInnerHTML={{ __html: p.editorial.content }} />
              </div>
            ) : (
              <div className="bg-surface border border-border-card rounded-2xl p-12 text-center">
                ...
                <p className="text-sm text-text-muted mt-1">This problem does not have an official editorial or solution guide.</p>
              </div>
            )}

            {p.solutions && p.solutions.length > 0 && (
              <div className="bg-surface border border-border-card rounded-2xl p-6">
                <h3 className="font-semibold text-text-primary mb-4">Reference Solutions (Judge)</h3>
                <div className="space-y-4">
                  {p.solutions.map((s, i) => (
                    <div key={i}>
                      <Pill tone="green" className="mb-2">{s.language}</Pill>
                      <pre className="p-3 bg-background/60 rounded-lg border border-border-card font-mono text-[11px] text-text-secondary overflow-x-auto">{s.solution}</pre>
                    </div>
                  ))}
                </div>
              </div>
        ...
        {activeTab === "analytics" && (
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              <StatCard label="Total Submissions" value={isSubsLoading ? "..." : formatNumber(totalSubs)} />
              <StatCard label="Accepted" value={isSubsLoading ? "..." : formatNumber(acceptedSubs.length)} />
              <StatCard label="Acceptance Rate" value={isSubsLoading ? "..." : `${acceptanceRate}%`} />
              <StatCard label="Unique Solvers" value={isSubsLoading ? "..." : formatNumber(uniqueSolvers)} />
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              ...
              <div className="lg:col-span-2 bg-surface border border-border-card rounded-2xl overflow-hidden flex flex-col">
                <div className="p-4 border-b border-border-card bg-background/30 flex justify-between items-center">
                  ...
                  <Link to="/submissions" search={{ problemExternalId: p.externalId }} className="text-xs text-brand-primary hover:underline">
                    View All
                  </Link>
                </div>
                <div className="p-0 flex-1 overflow-auto">
                  <DataTable
                    ...
                    columns={subColumns}
                    emptyMessage={isSubsLoading ? "Loading submissions..." : "No submissions for this problem yet."}
                  />
                </div>
              </div>
            </div>
            ...
            <div className="bg-surface border border-border-card rounded-2xl p-6 space-y-4">
              ...
              <div className="space-y-3 pt-2">
                ...
                <div className="pt-4 border-t border-border-card mt-4">
                  <button 
                    onClick={() => {
                      if (confirm("Are you sure you want to deactivate this problem? It will no longer be visible to users, but their submissions will be retained.")) {
...
function StatCard({ label, value }: { label: string; value: string | number }) {
  return (
    <div className="bg-surface border border-border-card rounded-xl p-4 flex flex-col justify-center items-center text-center">
      <span className="text-[11px] text-text-muted uppercase tracking-wider font-semibold mb-1">{label}</span>
